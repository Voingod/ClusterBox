...
{
    public partial class RegressionWindow : Form
    {
       ...
       // string[] filterCl = new string[] { };
        ...
        void FilterClCount(int k, int z, List<string> Surname)
        {
            //DataView dv = new DataView(dt);//Создание новой таблицы
        ...
        //Считает колличество переходов из класстера в кластер. Был студент в 1 кластере, по метке шел во второй, если перешел, тогда +1
        private void btnCountInCl_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Surname.Count; i++)
            {
                ...
                for (int j = 0; j < num; j++)
                {
                    if ((j + 1) < num)
                    {
                        ...
                        {
                           // Console.WriteLine(Surname[i] + " " + dtSurname.Rows[j][23].ToString() + " " + dtSurname.Rows[j][24].ToString());
                            //Console.WriteLine(Surname[i] + " " + dtSurname.Rows[j+1][23].ToString() + " " + dtSurname.Rows[j+1][24].ToString());
                            ClusterVecCount++;
                            // dataGridViewCountInCl[Convert.ToInt32(dtSurname.Rows[j][23].ToString()), Convert.ToInt32(dtSurname.Rows[j][24].ToString())].Value =1;
                            //dataGridViewCountInCl[0, 0].Value = 0;
                            ClCount[Convert.ToInt32(dtSurname.Rows[j][23].ToString())-1, Convert.ToInt32(dtSurname.Rows[j][24].ToString())-1] += 1;
                        }
                    }
                }
            }
            ...
            lblCountInCL.Text = Convert.ToString(ClusterVecCount);
